...
@section('content')
<div class="p-6">
    <h1 class="text-2xl font-bold text-gray-800 mb-1">Edit Data RT</h1>
    <p class="text-gray-500 mb-6">Form untuk mengubah data Rukun Tetangga (RT)</p>
        ...
        <form action="{{ route('rt.update', $rt->rt_id) }}" method="POST">
                ...
                <select name="rw_id"
                    ...
                    @foreach ($rwList as $rw)
                        <option value="{{ $rw->rw_id }}" 
                                {{ (old('rw_id', $rt->rw_id) == $rw->rw_id) ? 'selected' : '' }}>
                            RW {{ $rw->nomor_rw }}
                            @if($rw->ketuaRw)
                ...
                <select name="ketua_rt_warga_id"
                    ...
                    @foreach ($warga as $w)
                        <option value="{{ $w->warga_id }}" 
                                {{ (old('ketua_rt_warga_id', $rt->ketua_rt_warga_id) == $w->warga_id) ? 'selected' : '' }}>
                            {{ $w->nama }} - {{ $w->no_ktp }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea name="keterangan" rows="3"
                          class="w-full border px-3 py-2 rounded-lg focus:ring focus:ring-green-200">{{ old('keterangan', $rt->keterangan) }}</textarea>
            </div>
