...
<div class="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow">
    <h1 class="text-2xl font-bold mb-6 text-gray-800">Edit Anggota Lembaga</h1>
    ...
    <form action="{{ route('anggota-lembaga.update', $anggotaLembaga->anggota_id) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')
            ...
            <select name="lembaga_id" class="w-full border rounded px-3 py-2" required>
                @foreach($lembagas as $lembaga)
                    <option value="{{ $lembaga->lembaga_id }}"
                        {{ old('lembaga_id', $anggotaLembaga->lembaga_id) == $lembaga->lembaga_id ? 'selected' : '' }}>
            ...
            <select name="warga_id" class="w-full border rounded px-3 py-2" required>
                @foreach($wargas as $warga)
                    <option value="{{ $warga->warga_id }}"
                        {{ old('warga_id', $anggotaLembaga->warga_id) == $warga->warga_id ? 'selected' : '' }}>
            ...
            <select name="jabatan_id" class="w-full border rounded px-3 py-2" required>
                @foreach($jabatans as $jabatan)
                    <option value="{{ $jabatan->jabatan_id }}"
                        {{ old('jabatan_id', $anggotaLembaga->jabatan_id) == $jabatan->jabatan_id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="date" name="tgl_mulai"
                   value="{{ old('tgl_mulai', $anggotaLembaga->tgl_mulai) }}"
        ...
        <div>
            ...
            <input type="date" name="tgl_selesai"
                   value="{{ old('tgl_selesai', $anggotaLembaga->tgl_selesai) }}"
